...
**Language beachheads:** user-service is **ASP.NET Core 8**; integration-service and notification-service are **Spring Boot** (Docker on Render). Shared Postgres client knobs (`DB_POOL_*` / `DB_RETRY_*`) — apply the same names when rewriting or hardening other services ([env standard](../configuration/environment-variables.md#database-client-pool--retry-standard)).
