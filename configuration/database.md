...
3. Pick mode by client runtime:
   ...
   |--------|--------|----------------|-----|
   | **user-service (C# / Npgsql)** | **Session** pooler | `5432` on `*.pooler.supabase.com` | Transaction mode (`6543`) caused Npgsql read timeouts; default `DB_SUPABASE_POOL_6543TRANS_5432SESSION=5432SESSION` forces session port on the pooler host |
