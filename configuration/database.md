...
3. Pick mode by client runtime:
   ...
   |--------|--------|----------------|-----|
   | **user-service (C# / Npgsql)** | **Session** pooler | `5432` on `*.pooler.supabase.com` | Transaction mode (`6543`) caused Npgsql read timeouts; default `DB_SUPABASE_POOL_6543_4TR_5432_4SESN=5432` forces session port on the pooler host |
